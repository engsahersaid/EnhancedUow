...
namespace UOwPoc.Core.Services
{
    public class PersonService : IPersonService
    {
        ...
        public List<PersonVM> GetByNationality(int nationalityId)
        {
            //var specification = new PersonNationalitySpecifcation(nationalityId);
            //var basespecification = specification.Or(new PersonIsAdultSpecification());
            var persons = _uow.QueryRepository<Person>().FindWithSpecificationPattern(new PersonIsAdultSpecification());
            return _mapper.Map<List<PersonVM>>(persons);
        }
    }
}